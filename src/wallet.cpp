...
/**
 * Fees smaller than this (in uamnz) are considered zero fee (for transaction creation)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_NOT10000IFMN) {
                    found = !(fMasterNode && masternodeSigner.IsCollateralAmount(pcoin->vout[i].nValue));
                } if (nCoinType == ONLY_10000) {
                    found = masternodeSigner.IsCollateralAmount(pcoin->vout[i].nValue);
                } else {
                    found = true;
                }
                if (!found) continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 AMNZ.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-amnz-address
                    CScript scriptChange;
